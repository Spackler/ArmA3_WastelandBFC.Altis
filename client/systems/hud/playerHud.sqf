...
while {true} do
{
	...
	if (_atmEnabled) then {
		_strArray pushBack format ["%1 <img size='1.0' image='client\icons\bank.paa'/>", [player getVariable ["bmoney", 0]] call fn_numbersText];
	};

	_strArray pushBack format ["%1 <img size='1.0' image='client\icons\money.paa'/>", [player getVariable ["cmoney", 0]] call fn_numbersText];
	...
	_strArray pushBack format ["<t color='%1'>%2</t> <img size='1.1' image='client\icons\health.paa'/>", _healthTextColor, _health];
